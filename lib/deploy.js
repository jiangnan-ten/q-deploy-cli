...
class Deploy {
	...
	async run() {
		...
		try {
			...
			this.log('本地不存在打包文件, 请先打包, 再上传')
		}

		await this.sshConnet()
	}

	log(msg, customMsg = false, isExit = true, sign = '❗') {
	...
	async prompts() {
		...
		if (!this.env) {
			...
			try {
				...
				this.log('配置文件, env环境参数不正确')
			}

			if (!choices.length) {
				this.log('配置文件, 缺失env环境参数')
			}
	...
	buildProject() {
		...
		return new Promise(async (resolve) => {
			...
			try {
				let res = await execa.command(buildScript, { cwd: process.cwd() })
				console.log(chalk.cyan(res.stdout))
			} catch (error) {
				console.log('')
				this.log(chalk.yellow(error.message), true, false)
				console.log('')
				this.log(chalk.red('打包失败, 部署终止'), true, true, '⛔')
			}
	...
	async sshConnet() {
		...
		console.log('')
		this.log(chalk.green('恭喜你, 部署完成!!!'), true, true, '🎉')
	}
}
